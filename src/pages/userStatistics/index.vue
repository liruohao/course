<template>
    <div class="salesStatistics">
      <h4 style="margin-bottom: 10px">总计用户访问量：{{num}}</h4>
      <div style="display: flex;padding-left: 8%;height: 30px; justify-content: space-between">
        <div>
          <Button type="primary" style="margin-right: 10px" @click="getCountList('day')" v-if="ecFlag">最近七日用户访问量</Button>
          <Button type="primary" style="margin-right: 10px" @click="getCountList('day')" v-else>最近七日用户访问趋势</Button>
          <Button type="primary" style="margin-right: 10px" @click="getCountList('month')" v-if="ecFlag">今月用户访问量</Button>
          <Button type="primary" style="margin-right: 10px" @click="getCountList('month')" v-else>今月用户访问趋势</Button>
          <Button type="primary"  @click="getCountList('year')"  v-if="ecFlag">今年用户访问量</Button>
          <Button type="primary"  @click="getCountList('year')"  v-else>今月用户访问趋势</Button>
        </div>
        <Button type="warning"  @click="ecFlag =!ecFlag" v-if="ecFlag">切换访问趋势</Button>
        <Button type="warning"  @click="ecFlag =!ecFlag" v-else>切换访问量</Button>
      </div>
      <div
        class="PM10-echarts"
        id="UserChart"
...
export default {
  ...
  methods: {
    ...
    getCountListAll (data = 'day') {
      this.$http.get('sys-count/getCountListAll', {
        countType: data
      }, res => {
        this.num = res.data
      })
    ...
    getCountList (data = 'day') {
      this.$http.get('sys-count/getCountList', {
        countType: data
      }, res => {
